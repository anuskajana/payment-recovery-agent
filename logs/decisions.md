...
## 2026-08-29 — Real API integrations wired in
Switched LLM escalation from Anthropic to Gemini (had a Gemini key available
first). Fixed several indentation bugs introduced during copy-pasting between
editors - a good reminder that Python's whitespace sensitivity makes manual
code editing across tools risky; full-file replacement was safer than patching
fragments once things got tangled.

Added src/razorpay_integration.py as a small, separate module that makes a
real API call to Razorpay's test-mode Orders endpoint - creates and fetches
back a genuine order (order_TVtg0KmbKA39wz, verified in the Razorpay test
dashboard). This is deliberately kept separate from simulator.py: the
simulator handles realistic batch-scale testing (50+ synthetic records) for
the rule engine and LLM layers, while this module proves the project actually
integrates with Razorpay's real platform, not just local fake data.
